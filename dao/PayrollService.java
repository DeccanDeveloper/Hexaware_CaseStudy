...
public class PayrollService implements IPayrollService {
    ...
    public double calculateGrossSalary(int employeeId) {
        String sql = "SELECT BasicSalary, OvertimePay FROM Payroll WHERE EmployeeID = ? ORDER BY PayPeriodEndDate DESC LIMIT 1";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, employeeId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public double calculateNetSalary(int employeeId) {
        String sql = "SELECT BasicSalary, OvertimePay, Deductions FROM Payroll WHERE EmployeeID = ? ORDER BY PayPeriodEndDate DESC LIMIT 1";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, employeeId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
